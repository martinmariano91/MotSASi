    ...
    for i in range(len(mot)): 
        ...
        for e in foldx.columns:
            if i in pos_x: # check if is a flexible position
                ...
                elif foldx.iloc[i][e] > 1 and foldx.iloc[i][e] > row_mean: 
                        for x in d.values():
                            pathogenics.append(x+str(i+1)+d[e])
                else:
...
#%%                
    ...
    with open('../MotListos_1er_etapa/'+motif_name_folder+'/'+motif_name+'_Validation_trues1.txt', "w") as output:
        output.write(str(true_motifs['ID_motif'].tolist()))
        
    with open('../MotListos_1er_etapa/'+motif_name_folder+'/'+motif_name+'_Validation_falses1.txt', "w") as output:
        output.write(str(false_motifs['ID_motif'].tolist()))
        
    with open('../MotListos_1er_etapa/'+motif_name_folder+'/'+motif_name+'_Validation_conflicts1.txt', "w") as output:
